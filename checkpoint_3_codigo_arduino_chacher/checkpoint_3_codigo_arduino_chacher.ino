...
const char* ssid     = "Fibertel WiFi836 2.4GHz"; // "BA Escuela";
...
float temperature = 23.0; // Assume current temperature. Recommended to measure with DHT22
float humidity = 60.0; // Assume current humidity. Recommended to measure with DHT22
int medicion = map(((analogRead(PIN_MQ135) - 20) * 3.04), 0, 1023, -40, 125);

const unsigned char epd_bitmap_calavera24 [] PROGMEM = {
...
void loop() {
  ...
  if (modoFacil){
    ...
    display.print(" ppm");
    display.drawRect(10, 12, 80, 40, SSD1306_WHITE);
    // Chequeo de alarmas para printear diferentes símbolos
    if (alarma == 2){
      display.drawBitmap(95, 20, epd_bitmap_calavera24, 25, 24, SSD1306_WHITE);
    }else if (alarma == 0){
  ...
  //
  if (digitalRead(PIN_PULSADOR) == LOW && (millis() - delayPulsador >= 50)){
    modoFacil = !modoFacil;
